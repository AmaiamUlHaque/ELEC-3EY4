...
class RacecarExperiment {
    ...
    double speed_to_erpm_gain, speed_to_erpm_offset, steering_angle_to_servo_gain, steering_angle_to_servo_offset, wheelbase;
    ...
    RacecarExperiment() {
        ...
        // odom_imu_transform.child_frame_id = base_frame;



        // Make a publisher for odometry messages
        odom_pub = n.advertise<nav_msgs::Odometry>(odom_topic, 1);
        // odom_imu_pub = n.advertise<nav_msgs::Odometry>(odom_imu_topic, 1);
    ...
    void vesc_callback(const vesc_msgs::VescStateStamped & state){
        ...
        double delta_x = current_speed * std::cos(yaw) * dt.toSec();
        double delta_y = current_speed * std::sin(yaw) * dt.toSec();
        x_odom += delta_x;
        y_odom += delta_y;
        ...
        // Broadcast odomtery transfomration 
    ...
    void laser_callback(const sensor_msgs::LaserScan & msg) {
        ...
        std::vector<float> scan_ranges(n);
        std::vector<float> scan_intensities(n);
    
        // Swap the two halves of the data (with middle element unchanged if odd)
      ...
      if (n % 2 == 0) {
        ...
        for (size_t i = 0; i < half_n; ++i) {
            ...
            // Swap the ranges
            scan_ranges[i] = msg.ranges[j];
            scan_ranges[j] = msg.ranges[i];
    
            // Swap the intensities
            scan_intensities[i] = msg.intensities[j];
            scan_intensities[j] = msg.intensities[i];
            }
          }
     ...
     else
          {
            for (size_t i = 0; i < half_n; ++i) {
                ...
                // Swap the ranges
                scan_ranges[i] = msg.ranges[j];
                scan_ranges[j+1] = msg.ranges[i];
        
                // Swap the intensities
                scan_intensities[i] = msg.intensities[j];
                scan_intensities[j+1] = msg.intensities[i];
                }
                scan_ranges[half_n] = msg.ranges[n-1];
                scan_intensities[half_n] = msg.intensities[n-1];
              } 
    
        // Prepare and publish the new message

        sensor_msgs::LaserScan scan_msg;
        scan_msg.header.stamp = msg.header.stamp;
        scan_msg.header.frame_id = msg.header.frame_id;
        scan_msg.angle_min = msg.angle_min+ M_PI;
        scan_msg.angle_max =  msg.angle_max+ M_PI;
        scan_msg.angle_increment = msg.angle_increment;
        scan_msg.range_max = msg.range_max;  
        scan_msg.ranges = std::move(scan_ranges);
        scan_msg.intensities = std::move(scan_intensities);
	...
	void imu_callback(const sensor_msgs::Imu& imu_msg) {
        ...
        // The following IF statment is meant to catch the initial IMU reading at t=0, init_yaw. 
        // This offest should be subtracted from future readings to compute vehicle yaw angle
